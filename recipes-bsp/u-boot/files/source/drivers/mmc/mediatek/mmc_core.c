...
static int mmc_send_op_cond(struct mmc_host *host, u32 ocr, u32 *rocr)
{
    ...
    for (i = 100; i; i--) {
        err = mmc_cmd(host, &cmd);
        if (err != MMC_ERR_NONE) {
	        MSDC_TRC_PRINT(MSDC_INIT,("[%s]: Command failed: %d\n", __func__, err));
            break;
		}

        /* if we're just probing, do a single pass */
        if (ocr == 0) {
	        MSDC_TRC_PRINT(MSDC_INIT,("[%s]: ending after a single pass... \n", __func__));
            break;
		}

        if (cmd.resp[0] & MMC_CARD_BUSY) {
	        MSDC_TRC_PRINT(MSDC_INIT,("[%s]: response OK\n", __func__));
			err = MMC_ERR_NONE;
            break;
		}
    ...
    MSDC_TRC_PRINT(MSDC_INIT,("[%s]: exiting, returning %d\n", __func__, err));
...
static int mmc_send_app_op_cond(struct mmc_host *host, u32 ocr, u32 *rocr)
{
    ...
    for (i = 100; i; i--) {
        ...
        if (cmd.resp[0] & MMC_CARD_BUSY || ocr == 0) {
	        MSDC_TRC_PRINT(MSDC_INIT,("[%s]: response OK or single pass, returning\n", __func__));
			err = MMC_ERR_NONE;
            break;
		}
...
int mmc_init_card(struct mmc_host *host, struct mmc_card *card)
{
    ...
    MSDC_TRC_PRINT(MSDC_INIT,("[%s]: start (id: %d)\n", __func__, id));
    ...
    MSDC_TRC_PRINT(MSDC_INIT,("[%s]: idle result: %d\n", __func__, err));

    /* send interface condition */
    err = mmc_send_if_cond(host, host->ocr_avail);
    MSDC_TRC_PRINT(MSDC_INIT,("[%s]: send_if_cond result: %d (failures can be ignored)\n", __func__, err));
    ...
    MSDC_TRC_PRINT(MSDC_INIT,("[%s]: send_app_op_cond result: %d (ignore failures)\n", __func__, err));

    if (err != MMC_ERR_NONE) {
        err = mmc_send_op_cond(host, 0, &ocr);
        MSDC_TRC_PRINT(MSDC_INIT,("[SD%d] send_op_cond result: %d  (in MMC_CMD_SEND_OP_COND/SD_ACMD_SEND_OP_COND cmd)\n", id, err));
        if (err != MMC_ERR_NONE)
            goto out;
    ...
    MSDC_TRC_PRINT(MSDC_INIT,("[%s]: send_if_cond result: %d\n", __func__, err));
...
int mmc_init_host(struct mmc_host *host, int id)
{
    ...
    MSDC_TRC_PRINT(MSDC_INFO,("[%s]: start\n", __func__));
...
int __mmc_init(int id, block_dev_desc_t *bdev)
{   
    int err = MMC_ERR_NONE;
    struct mmc_host *host;
    struct mmc_card *card;
    ...
    MSDC_TRC_PRINT(MSDC_INFO,("[%s]: mmc%d mmc_init_host() finished (%d: %s)\n", 
		__func__, id, err, (err == MMC_ERR_NONE) ? "OK" : "Error"));
	if (err != MMC_ERR_NONE)
		return err;
    
	err = mmc_init_card(host, card);
    MSDC_TRC_PRINT(MSDC_INFO,("[%s]: mmc%d mmc_init_card() finished (%d: %s)\n", 
		__func__, id, err, (err == MMC_ERR_NONE) ? "OK" : "Error"));
	if (err != MMC_ERR_NONE)
		return err;

    MSDC_TRC_PRINT(MSDC_INIT,("[%s] mmc%d, Size: %d MB, Max.Speed: %d kHz, blklen(%d), nblks(%d), ro(%d)\n",
		__func__, id, ((card->nblks / 1024) * card->blklen) / 1024 , card->maxhz / 1000,
        card->blklen, card->nblks, mmc_card_readonly(card)));

    bdev->part_type   = PART_TYPE_EFI;
    bdev->dev         = id;
    bdev->removable   = (id == 0) ? 0 : 1;
    bdev->lba         = card->nblks * card->blklen / bdev->blksz;

	sprintf(bdev->vendor, "Man %06x Snr %04x%04x",
		card->raw_cid[0] >> 24, (card->raw_cid[2] & 0xffff),
		(card->raw_cid[3] >> 16) & 0xffff);
	sprintf(bdev->product, "%c%c%c%c%c%c", card->raw_cid[0] & 0xff,
		(card->raw_cid[1] >> 24), (card->raw_cid[1] >> 16) & 0xff,
		(card->raw_cid[1] >> 8) & 0xff, card->raw_cid[1] & 0xff,
		(card->raw_cid[2] >> 24) & 0xff);
	sprintf(bdev->revision, "%d.%d", (card->raw_cid[2] >> 20) & 0xf,
		(card->raw_cid[2] >> 16) & 0xf);

    MSDC_TRC_PRINT(MSDC_INIT,("[%s]: vendor: %s\n", 
		__func__, bdev->vendor ));
    MSDC_TRC_PRINT(MSDC_INIT,("[%s]: product: %s, revision: %s\n", 
		__func__, bdev->product, bdev->revision ));
